    ...
    public class MemberAutoNSubstituteDataAttributeTests
    {
        ...
        public void GivenExistingMemberName_WhenGetDataIsInvoked_ThenAppropriateDataIsReturned()
        {
            ...
            for (var i = data.Count - 1; i >= 0; i--)
            {
                ...
                result[numberOfParameters - 1].GetType().Name.Should().StartWith("ObjectProxy", "that way we know it was mocked with NSubstitute.");
            }
        }
        ...
        public void GivenTestMethodHasSomeMemberGeneratedParameters_WhenTestRun_ThenParametersAreProvided(int first, int second, int third, int fourth, IDisposable disposable)
        {
            ...
            disposable.GetType().Name.Should().StartWith("ObjectProxy", "that way we know it was mocked with NSubstitute.");
        }
    }
}